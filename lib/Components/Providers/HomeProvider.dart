...
class HomeProvider with ChangeNotifier {
  ...
  String user_fingerprint =
      'NONE'; //Will hold the single user fp that can also be found in the user account details var
  ...
  int selectedDropoffPackageIndex =
      0; //Will hold the index of the package selected whent the driver wants to confirm the drop off
  ...
  void updateTripRequestsMetadata({required List<dynamic> newTripList}) {
    ...
    //...
    else if (newTripList.isEmpty) //NO rides
    {
      tripRequestsData = newTripList;
      notifyListeners();
    }
  ...
  void updateOnlineOfflineData({required Map<dynamic, dynamic> data}) {
    ...
    //!Auto update the selected option
    selectedOption = data['operation_clearance'] != null
        ? data['operation_clearance'].toString().toLowerCase()
        : 'none';
    //...
    notifyListeners();
  }
  ...
  //?35. Get clean payment method name
  ...
  //?36. Update the selected dropoff index for the package - delivery
  void updateSelectedPackageIndexForDelivery({required int index}) {
    selectedDropoffPackageIndex = index;
    notifyListeners();
  }
